...
function initialiseExperience() {
  ...
  initialiseOpenWhenLetters();
  initialiseVibeCheck();
  initialiseHeartArcade();
...
function startCountdown() {
  ...
  function update() {
    const now = new Date();
    const isBirthday = now.getMonth() === 7 && now.getDate() === 3;
    // On August 3rd the counter celebrates the remaining birthday hours;
    // every other day it points to the next local August 3rd.
    let birthday = new Date(now.getFullYear(), 7, 3, 0, 0, 0);
    if (isBirthday) {
      birthday = new Date(now.getFullYear(), 7, 4, 0, 0, 0);
      document.querySelector("#countdown-title").innerHTML = "Today is<br /><em>your day.</em>";
      document.querySelector(".countdown-section__date").innerHTML = "03 AUGUST <span>•</span> HAPPY BIRTHDAY, SADHANA";
    } else if (now >= birthday) birthday = new Date(now.getFullYear() + 1, 7, 3, 0, 0, 0);
...
/* ---------- 5. Private letters, birthday quiz, and tap arcade ---------- */
function initialiseOpenWhenLetters() {
  const dialog = $("#letter-dialog");
  const title = $("#dialog-title");
  const body = $("#dialog-body");
  const close = () => dialog.close();
  document.querySelectorAll(".open-letter").forEach((letter) => {
    letter.addEventListener("click", () => {
      title.textContent = letter.dataset.letterTitle;
      body.textContent = letter.dataset.letterBody;
      if (dialog.showModal) dialog.showModal();
    });
  });
  $("#close-letter").addEventListener("click", close);
  dialog.addEventListener("click", (event) => {
    if (event.target === dialog) close();
  });
}

function initialiseVibeCheck() {
  const questions = [
    { title: "Choose today’s<br /><em>energy.</em>", answers: ["Stargazing and soft music", "Cake before dinner", "Dancing like nobody’s watching"] },
    { title: "Pick a birthday<br /><em>superpower.</em>", answers: ["Pause every perfect moment", "Make wishes come true", "Turn bad days into good ones"] },
    { title: "Final choice:<br />how bright should<br />your year be?", answers: ["A little brighter", "A whole galaxy", "As bright as your smile"] },
  ];
  const heading = $("#vibe-check-title");
  ...
  function renderQuestion() {
    ...
    step.textContent = `Question ${String(current + 1).padStart(2, "0")} / 03`;
    heading.innerHTML = question.title;
    options.innerHTML = question.answers.map((answer) => `<button type="button">${answer}</button>`).join("");
    result.textContent = "No wrong answers. It is your day.";
    options.querySelectorAll("button").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        if (current < questions.length - 1) {
          result.textContent = "Excellent choice. Loading the next little question...";
          window.setTimeout(() => { current += 1; renderQuestion(); }, prefersReducedMotion ? 1 : 750);
        } else {
          step.textContent = "RESULT / VERIFIED";
          heading.innerHTML = "Vibe check<br /><em>complete.</em>";
          result.textContent = "100% Sadhana. Entirely brilliant. Completely unforgettable. ♥";
          result.classList.add("is-final");
          options.innerHTML = "";
          createWishParticles($("#vibe-check-title"), 14);
        }
      });
    });
  }
...
function initialiseHeartArcade() {
  const game = $("#heart-game");
  const playfield = $("#game-playfield");
  const start = $("#game-start");
  const scoreNode = $("#game-score");
  const timeNode = $("#game-time");
  ...
  function stopGame() {
    ...
    playfield.querySelectorAll(".game-heart").forEach((heart) => heart.remove());
    start.hidden = false;
    start.textContent = "Play again ▶";
    message.textContent = score >= 10 ? "Mission complete: bonus love unlocked ♥" : `Mission paused at ${score} hearts. Try once more?`;
    if (score >= 10) {
  ...
  function spawnHeart() {
    ...
    heart.style.setProperty("--fall-duration", `${2.4 + Math.random() * 1.9}s`);
    heart.addEventListener("click", () => {
      if (!running) return;
      score += 1;
      scoreNode.textContent = String(score).padStart(2, "0");
      createWishParticles(heart, 3);
      heart.remove();
      if (navigator.vibrate) navigator.vibrate(10);
      if (score >= 10) stopGame();
    }, { once: true });
    heart.addEventListener("animationend", () => heart.remove(), { once: true });
    playfield.append(heart);
  }
  ...
  start.addEventListener("click", () => {
    ...
    timeNode.textContent = "20s";
    message.textContent = "Tap fast — the universe is counting on you.";
    start.hidden = true;
    spawnHeart();
    spawnId = window.setInterval(spawnHeart, 620);
    timerId = window.setInterval(() => {
      time -= 1;
      timeNode.textContent = `${time}s`;
      if (time <= 0) stopGame();
    }, 1000);
  });
}
...
/* ---------- 7. Celebration: confetti, floating hearts, Web Audio ---------- */
